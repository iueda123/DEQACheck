...
* Axis 1. Modality: sMRI, fMRI, dMRI, MEG/EEG, PET, UNKNOWN
...
|               | sMRI                | VQ-VAE-TS          | vector-quantized variational autoencoder typicality score | VQ-VAEでの典型度スコア                      | 典型度z-score                |
|               | sMRI                | PBSI               | person-based similarity index                             | 溝幅類似度の個人指標                          | Sulcal width PBSI         |
|               | sMRI                | MSI                | morphometric similarity index                             | 形態類似度の指標                            | MSI行列の平均                  |
|               | fMRI, dMRI, MEG/EEG | NetMes             | network measures                                          | DC (degree centrality) などのネットワーク特徴量 | 視床のDC                     |
...
VBMの「モジュレーション」は、非線形正規化の変形場で得たJacobian決定量を各ボクセルに掛ける体積補正のことである。
テンプレートに合わせて引き伸ばし・縮みが生じた分を補正し、正規化前の局所体積を保ったボクセル値（modulated GMV/WMV）として扱う。
「deformation由来の形態指標をNormative Modelingした研究」というニュアンスが伝わる名前で括りを設けるのが適切であろう。
...
- Fraza2023（UKB Jacobianモデル）: 
  Cam-CAN/HCP/OASIS/PNC/UKBの19,620人から得たT1→MNIのJacobian determinants（voxel-wise）をWarped BLRでnormativeモデル化し、CNVキャリアの逸脱Zを解析。
...
## "Axis 1. Modality" と "Keyword" の対応関係の適切さに関する確認
