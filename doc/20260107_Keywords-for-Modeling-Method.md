...
| Nonparametric Reg | MAA            | moving average approach                                   | 移動平均による時系列の平滑化                                                         |                            |
...
| Others            | NIE            | Nonparametric Interval Estimation                         | nonparametric two-sided tolerance intervals 分布仮定なしの両側許容区間推定 (Fang2025) |
...
- **GPR**: Bayer2022, Chan2025B, Fang2024, Floris2021, Geng2025, Han2023, Han2024A, Han2024B, Holz2023, Huo2024,
  Ilioska2024, Jia2024, Jia2025, Laidi2022, Liu2024, Parkes2021, Wolfers2018, Wolfers2020, Wolfers2021, Zabihi2019,
  Zabihi2020 など（31研究）
- **WBLR**: Bhome2024, Cirstian2024, Echave2024, Fraza2023, Haukvik2025, Janssen2024, Kasper2024, Meijer2024,
  RehakBuckova2025, Rutherford2022, Rutherford2023, Segal2025, Verdi2024 など（16研究）
- **HBR (HBLM/HBGPM含む)**: Berthet2025, Kia2022, Lawn2024, Loreto2024, Mansour2025, Segal2023, Sun2025, Verdi2023,
  VillalonReina2024, Worker2023, Wu2023, Wu2024（12研究）
...
- Feng2024, Feng2025, Kumar2024, Kumar2025, Mendes2024, OliveiraSaraiva2023, Pinaya2019, Pinaya2021, Sampaio2025,
  Tong2024, Vieira2025（11研究）
...
- **PBSI (Similarity-Based Methods)**: Baldwin2022, Joo2024（2研究）  
  例: Baldwin2022 はSA/CT/SV/ICVの153 ROIプロファイルをHC平均と相関させるPBSIを算出し、個人の逸脱度をZ化。Joo2024 では308
  ROI（SA/GMV/CT/LGI）のPBSIで逸脱者を特定。
...
- **Leenings2024**: Structural MRIの脳形態（加齢・疾患）に対し、Nearest Neighbor Normativity (N³)
  を主要手法として導入し、個別の正常性参照を近傍サンプルで構築。
...
- **NWK (Nadaraya-Watson Kernel Regression)**: カーネル重み付き平均で条件付き期待値を推定するノンパラメトリック回帰。
- **QUANTREG (Quantile Regression)**: 分位点回帰。条件付き中央値や任意のパーセンタイルを直接推定。
- **MAA (Moving Average Approach)**: 一定幅の時間窓をスライドしながら平均との差を算出し、時系列を平滑化する。
...
- **GAMLSS**: Bedford2025, Bethlehem2021, Ebadi2024, GarciaSanMartin2025, Ge2024, Georgiadis2024, Huang2024, Lee2025,
  Leiberg2023, Little2025, Mao2025, Remiszewski2022, Tabbal2025, Wang2023, Young2024（15研究）  
...
- **Nadaraya-Watson**: Elad2021（1研究）  
  例: Elad2021 はGaussian kernelのNadaraya-Watson推定で年齢・性別ごとのnormative rangeを推定し、ROIごとのzスコアを算出。
...
- **Moving Average Approach**: Chien2022（1研究）  
  例: 観測データの短期ノイズをならし、トレンドやベースラインを抽出する前処理として移動平均を用いて、より安定した指標を得る。
...
- **Polynomial Regression**: Bethlehem2020, Coupe2022, Ge2024, Gordaliza2024, Haas2024, Jiang2024, Jing2023, Lin2023,
  Romascano2024, Yang2025, Zhang2023（11研究）  
...
- **GLM, OLSR, MFPR**: 多くの研究で基本手法として使用されているが、主要手法としての明示的な記載は限定的
    - **GLMの１例**: Jing2023 ではタスクfMRIコントラストのZマップを GLM で生成した後、行列を分解しスコアに変換して
      normative モデルに投入。
    - **OLSRの１例**: Jiang2024 では単純なOLS回帰で年齢・性別を共変量にした脳形態指標の予測式を作り、残差をZ化して逸脱度を計算。
    - **MFPRの１例**: Romascano2024 では分数次数の多項式（例: 年齢^0.5 や 年齢^-1 を含む）を使って複数の形態指標の非線形年齢効果を柔軟に近似し、normative
      分布を学習。


### Nonparametric Interval Estimation (NIE)

分布仮定を置かず、データの順序統計量に基づいて許容区間を推定する手法群。

**特徴:**

- 分布形状の仮定が不要
- 外れ値や非正規性に対して頑健
- 目的は回帰曲線の推定ではなく、個体が正常範囲内にあるかの区間推定
...
- 事前分布や関数形を仮定せずに正常範囲を定義できる
- 尾部の挙動に依存しない安定した区間推定が可能

**代表的な研究と具体例:**

- **Fang2025**: nonparametric two-sided tolerance intervals (サンプルから分布自由な両側許容区間を構成する方法。) を用いて許容区間を推定し、個体の逸脱を判定。
