...
def parse_age_record(r: str) -> str:
    ...
    for t in tokens:
        low=t.lower()
        if model is None and (re.search(r"\bnm\d+\b", low) or 'func' in low or 'structur' in low or 'smri' in low or 'dmri' in low):
    ...
    if rem2:
        ...
        if len(nums)==2 and vals['min']=='NR' and vals['max']=='NR' and vals['mean']=='NR' and vals['sd']=='NR':
...
def parse_sex_record(r: str) -> str:
    ...
    if m:
        ...
        if low=='model':
            if '|' in body:
                model=body.split('|',1)[0].strip(); body=body.split('|',1)[1].strip()
            else:
                model=body; body=''
    ...
    def find_one(letter):
        ...
        return None
    F = find_one('F'); M = find_one('M'); NR=find_one('NR')
    segs=[]
    ...
    if F is None and re.search(r"\bF\s*\d+\b", body):
        c=int(strip_commas(re.search(r"\bF\s*(\d+)\b", body).group(1))); F=(c,None)
    if M is None and re.search(r"\bM\s*\d+\b", body):
        c=int(strip_commas(re.search(r"\bM\s*(\d+)\b", body).group(1))); M=(c,None)
    if NR is None and re.search(r"\bNR\s*\d+\b", body):
        c=int(strip_commas(re.search(r"\bNR\s*(\d+)\b", body).group(1))); NR=(c,None)
    def fmt(pair,label):
    ...
    if sex_parts: segs.append(", ".join(sex_parts))
    else: segs.append(body)
    return " | ".join(segs)
    ...
    for key, func in (
        ...
        if isinstance(node, dict):
            ...
            if isinstance(ans,str) and ans.strip():
                new=func(ans)
                if new!=ans:
                    node['answer']=new; changed=True
        elif isinstance(node,str) and node.strip():
            new=func(node)
            if new!=node:
                ref[key]=new; changed=True
    if changed:
