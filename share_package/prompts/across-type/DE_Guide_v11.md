...
- Consistency and formatting
  ...
  - For ADCSL_Style, always return all five fields (answer, detail, confidence_rating, supporting_text, location).
...
There are four result styles used in this data extraction guide:
  ...
  * **ADCSL_Style** = Answer, Detail, Confidence rating, Supporting text, and Location style
    - Provide structured extraction including:
      - Answer: The extracted information
      - Detail: Concise, structured details required by the item (e.g., for model origin: pre-training dataset/source, modeling method, covariates, reference, and adaptation/calibration)
      - Confidence rating: High, Medium, or Low
      - Supporting text: Direct quotes from source materials
      - Location: Document location of the supporting text
    - Used when an item requires both a categorical answer and a compact descriptive detail
...
#### RCI-8. Quality Checking
  * Extraction Criteria: Presence of QC and key details
  * Extraction Result Style: ADCSL_Style
  * Notes: 
    * In Answer, choose one: "Yes" or "No".
    * In Detail, provide as applicable:
      ...
      - If QC is mentioned but details are minimal or referred to another paper, note that (with citation in Supporting Text/Location).
    * If Answer = "No", set Detail to "-".
  * Example:
...
#### RCI-9. Site Effect Handling
  * Extraction Criteria: Handling category and key details
  * Extraction Result Style: ADCSL_Style
  * Notes: 
    * In Answer, choose one: "None", "Batch-removal", "Model-based", or "Other".
    * In Detail, provide as applicable when Answer ≠ "None":
      - Method name (e.g., ComBat, ComBat-GAM, HBR/wBLR, transfer learning, calibration)
      - Batch variables used (site, center, dataset, scanner, vendor, protocol, etc.)
      - Covariates preserved (age, sex, diagnosis, etc.)
      - Whether approaches were in-sample or out-of-sample
...
For Extraction Items `RCI-1` 〜 `RCI-5`, `RCI-7`, `NM-3` 〜 `NM-11`, and `CAA-1` 〜 `CAA-6` (except `CAA-10`), follow the rules below when constructing answers, and return "ACRSL_Style" results. For `NM-1`, `RCI-8`, and `RCI-10`, return "ADCSL_Style" and follow the dedicated format below.
...
2. **Detail**: Concise, structured details required by the item.
   - For `NM-1`, include for Pre-trained models: size and source of the pre-training dataset (N, number of sites); modeling method used for pre-training; key covariates; reference to the original paper/resource; and whether/how the pre-trained model was adapted/calibrated for the current study. If Answer = New, set Detail to "-".
   - For `RCI-8`, include QC metrics, thresholds, exclusion counts/percentages, manual/automated procedures, whether QC metrics/exclusions were used as covariates, and any sensitivity analyses. If Answer = No, set Detail to "-".
   - For `RCI-10`, include method name, batch variables, covariates preserved, in-sample vs out-of-sample, transfer learning/calibration details, comparisons to alternatives, and validation of harmonization effectiveness. If Answer = None, set Detail to "-".
