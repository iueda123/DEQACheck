...
内容が固まったら各スキルを `.claude/commands/` 以下に個別ファイルとして配置する。

| ドラフト内容 | 配置先（スラッシュコマンド）                                                |
|:-------------|:----------------------------------------------------------------------------|
| SKILL 1      | `.claude/commands/prep-guide-for-de-guide.md` → `/prep-guide-for-de-guide` |
| SKILL 2      | `.claude/commands/prep-de-guide.md` → `/prep-de-guide`                     |
| SKILL 3      | `.claude/commands/gene-de-script.md` → `/gene-de-script`                   |

---

## SKILL 1: `/prep-guide-for-de-guide`

> **配置先**: `.claude/commands/prep-guide-for-de-guide.md`
...
> **配置先**: `.claude/commands/prep-de-guide.md`
...
> **配置先**: `.claude/commands/gene-de-script.md`
...
選択内容をまとめて表示し、ユーザーの確認を得てからスクリプト生成に進む。
...
前処理（アンカー付与）と情報抽出（AI呼び出し）を1本にまとめる。

**Step 1: 材料の前処理（スクリプト内に内包）**
...
- コピー先ファイルの各 Markdown 見出しに `{#anchor-id}` 形式のアンカーを自動付与
  （例: `## Methods` → `## Methods {#methods}`、`## Table 1` → `## Table 1 {#table-1}`）
- アンカーID は見出しテキストを lowercase・空白をハイフンに変換・英数字とハイフン以外を除去して生成

**Step 2: 情報抽出（AI呼び出し）**
- `share_package/tools/ask_AiToDe_v14_2.sh` の構造を参考にする
- 使用する `DE_Guide_{番号}.md` と関連ファイル（キーワードファイル等）をワークスペースにコピー
- 前処理済み材料を `ai_workspace/{timestamp}/study_1/` にコピーして AI を実行
- 出力 JSON のパス: `Studies/{AuthorYear}/extracted-info/{番号}/{AuthorYear}_by-{agent}-{model}_{yyyymmdd-HHmmss}.json`
- 出力ディレクトリが存在しない場合は自動作成
- デフォルトはドライラン（`--run` で実行モード）
