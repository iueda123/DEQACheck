...
1. **このガイドを読む**: まずこのファイル全体を読んで作業内容を理解してください
2. **全研究を確認**: すべての study_N/ フォルダの内容を確認してください
3. **横断的にデータ抽出**: 複数の研究を比較・参照しながら、各研究のデータを抽出してください
4. **結果を出力**: 各研究の結果を対応するJSONファイルに書き込んでください

**重要**:
- すべての study_N/ フォルダの内容を確認してから作業を開始してください
- 研究間で共通する情報や違いに注意し、回答形式を統一してください
...
```
<timestamp>/                          # カレントディレクトリ
├── DE_Guide_v11_studies.md           # このファイル（ガイド）
├── DE_<AuthorYear>_by_<agent>_*.json # 結果ファイル（各研究に1つ）
├── study_1/                          # 研究1の論文・資料
│   ├── <論文>.pdf.md
│   └── ...
├── study_2/                          # 研究2の論文・資料
...
- 各 study_N/ フォルダには1つの研究の論文と補足資料が含まれています
- 結果JSONファイルはカレントディレクトリ直下にあります（study_N/の中ではありません）
...
- 対象となる論文は study_1/, study_2/, ... study_N/ の各フォルダ下にあります
- これらの研究を横断的に参照しながら、次のセクションにある抽出依頼にある情報を抽出してください
