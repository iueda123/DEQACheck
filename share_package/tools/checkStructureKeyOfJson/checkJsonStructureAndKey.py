#!/usr/bin/env python3
# Usage:
#   python3 checkJsonStructureAndKey.py --data-type DE --target-folder PATH --template PATH [--output-folder PATH]
#   python3 checkJsonStructureAndKey.py --data-type QA --target-folder PATH --template PATH [--output-folder PATH]
#
# What it does:
...
# - Writes a summary TSV and per-file detail text outputs under the output folder (default: <script_dir>/check_rslt).
"""
JSON 構造・キー検査ツール
...
- 概要 TSV: 検査結果の一覧 (FAIL/WARN, 件数, 備考, ファイルパス)。
  デフォルトの出力先は "<スクリプトディレクトリ>/check_rslt" で、
...
def make_arg_parser(script_dir: Path) -> argparse.ArgumentParser:
    ...
    # 出力先のデフォルトは <スクリプトディレクトリ>/check_rslt。
    # 以下の2種類の成果物が作成されます:
    ...
    default_output = script_dir / "check_rslt"
