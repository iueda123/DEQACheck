...
オプション:
  ...
  -n, --name NAME        処理名を指定（結果ファイル名に使用；2025.12.26時点では "QA_v9" を指定）
...
使用例:
  ...
  ${this_script_name} --agent gemini --name QA_v9 --list tools/TaegetStudies.txt

  # Claudeを使用してQA処理を実行（複数の研究を対象）
  ${this_script_name} -a claude -n QA_v9 -l tools/TaegetStudies.txt

  # ドライラン（実行内容の確認のみ・デフォルト）
  ${this_script_name} -a codex -n QA_v9 -l tools/TaegetStudies.txt

  # 実行モードで処理を走らせる
  ${this_script_name} -a codex -n QA_v9 -l tools/TaegetStudies.txt -r
