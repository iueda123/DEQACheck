...
"""
Fill missing study_identification_part fields in Human JSONs from sibling codex JSONs.
...
Rules:
  - For each target file, if any of these fields are empty or missing,
    fill from a sibling JSON whose filename matches *codex*.json (case-insensitive):
      * study_identification_part/si1_study_id
      * study_identification_part/si2_reference_file_names
      * study_identification_part/si3_author_journal_year
      * study_identification_part/si4_title
      * study_identification_part/si5_doi
...
Usage:
  Simply run this script from repo root:
    python3 tools/fillJson_DESI_Part.py
"""
...
def is_empty(value: Any) -> bool:
    """Return True if the value should be considered empty and thus fillable.

    Special handling:
      - dict with 'answer' key follows emptiness of its 'answer'
      - list is empty if len==0 or all items empty
    """
    if value is None:
        return True
...
def write_json(path: Path, data: Dict[str, Any]) -> None:
    # Persist with UTF-8 and indentation
...
def find_codex_candidate(sibling_dir: Path, exclude: Path | None = None) -> Path | None:
    """Find a sibling *codex*.json (case-insensitive). If multiple, return newest by mtime."""
    cand: List[Path] = []
    # Collect all JSONs and filter by case-insensitive name contains 'codex'
...
def fill_from_codex(target_path: Path, *, dry_run: bool = False) -> Tuple[bool, List[str], Path | None]:
    """Fill missing fields in target_path from a sibling codex JSON.

    Returns (changed, updated_keys, codex_path)
    """
    changed = False
...
def build_arg_parser() -> argparse.ArgumentParser:
    desc = (
        "Human JSON の study_identification_part 欠損項目を、同階層の codex JSON から補完します。\n"
...
def main(argv: List[str]) -> int:
    ...
    # Discover target human JSON files
    ...
    targets: List[Path] = []
    for pat in patterns:
        # Use root.glob with a relative pattern from the root
    ...
    for idx, t in enumerate(targets, start=1):
        # Ensure consistent relative path display
        try:
