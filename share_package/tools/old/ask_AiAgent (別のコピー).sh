...
# ToDo:
#   * 複数のAIエージェントを並列で走らせた時に、生成ファイルを相互に参照してしまう可能性あり。修正したい。
#   * 同じAIに複数回かけられるようにしたい。
...
オプション:
  ...
  -n, --dry-run          実際には実行せず、実行予定のコマンドのみ表示
  -v, --verbose          詳細な出力を表示
...
使用例:
  ...
  # ドライラン（実行内容の確認のみ）
  ${this_script_name} -a codex -t DE -n

注意事項:
  - --agent と --type は必須オプションです
  - --study を指定しない場合、スクリプト内で定義された全研究を処理します
  - ドライランモードでは、実行されるコマンドが表示されますが実際には実行されません
...
if [[ "${DEorQA}" == "DE" ]]; then
    ...
    TemplateFile=${this_script_parent}/../templates/DE_Author20XX_by_Someone_YYYYmmddHHMMSS.json
else
...
for _author_year in ${AuthorYearArray[@]}; do
  ...
  if find ${_folder} -maxdepth 1 -type f -iname ${_expected_result_file} | grep -q .; then
      ...
      if [[ ${DryRun} == false ]]; then
          ...
          _result_file_name="${DEorQA}_${_author_year}_by_${AiAgentName}_YYYYMMDDHHMMSS.json"
      fi
      ...
      askAiAgent ${AiAgentName} ${GuideFile} ${_result_file_name} ${_run_agent}
      ...
      if [[ ${DryRun} == false ]]; then
          if [ "$(md5sum ${TemplateFile} | awk '{print $1}')" = "$(md5sum ${working_directory}/${_result_file_name} | awk '{print $1}')" ]; then
              ...
              echo ""
              echo "エラー: 結果ファイルが空です"
              rm ${working_directory}/${_result_file_name}
          else
              ...
              mv ${working_directory}/${_result_file_name} ${_dst}
          ...
          # Remove Guide File
          rm "${working_directory}/$(basename ${GuideFile})"
