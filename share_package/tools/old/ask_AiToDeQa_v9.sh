...
function askAiAgent(){
    ...
    if [[ ${_ai_agent_name} == "gemini" ]]; then
        ...
        if [[ ${_actual_run} == true ]]; then
            ...
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^" 2>&1 | tee -a ${_log_file_name}
            echo "Gemini による作業が完了しました"
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^" 2>&1 | tee -a ${_log_file_name}

        fi
    elif [[ ${_ai_agent_name} == "claude" ]]; then
        echo ""                                     2>&1 | tee -a ${_log_file_name}
        echo "========== Claude コマンド ==========" 2>&1 | tee -a ${_log_file_name}
        echo "claude -p \\"                         2>&1 | tee -a ${_log_file_name}
        echo "    \"$(basename ${_guide_file}) に従って作業をしてください。作業結果は ${_result_file_name} へ書き込んでください。\" \\" 2>&1 | tee -a ${_log_file_name}
        echo "    --allowedTools \"Bash,Read\" \\"  2>&1 | tee -a ${_log_file_name}
        echo "    --permission-mode acceptEdits"    2>&1 | tee -a ${_log_file_name}
        echo "====================================" 2>&1 | tee -a ${_log_file_name}
        echo ""                                     2>&1 | tee -a ${_log_file_name}
        ...
        if [[ ${_actual_run} == true ]]; then
            sleep 3
            echo "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" 2>&1 | tee -a ${_log_file_name}
            echo "Claude が作業を実行中..." 2>&1 | tee -a ${_log_file_name}
            echo "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" 2>&1 | tee -a ${_log_file_name}
            ...
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^" 2>&1 | tee -a ${_log_file_name}
            echo "Claude による作業が完了しました" 2>&1 | tee -a ${_log_file_name}
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^" 2>&1 | tee -a ${_log_file_name}

        fi
    elif [[ ${_ai_agent_name} == "codex" ]]; then
        echo ""                                     2>&1 | tee -a ${_log_file_name}
        echo "========== Codex コマンド =========="  2>&1 | tee -a ${_log_file_name}
        echo "codex exec --full-auto --skip-git-repo-check -C . \\" 2>&1 | tee -a ${_log_file_name}
        echo "    \"$(basename ${_guide_file}) に従って作業をしてください。作業結果は ${_result_file_name} へ書き込んでください。\""                            2>&1 | tee -a ${_log_file_name}
        echo "===================================="  2>&1 | tee -a ${_log_file_name}
        echo ""                                      2>&1 | tee -a ${_log_file_name}
        ...
        if [[ ${_actual_run} == true ]]; then
            sleep 3
            echo "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" 2>&1 | tee -a ${_log_file_name}
            echo "Codex が作業を実行中..." 2>&1 | tee -a ${_log_file_name}
            echo "vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv" 2>&1 | tee -a ${_log_file_name}
            ...
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^" 2>&1 | tee -a ${_log_file_name}
            echo "Codex による作業が完了しました" 2>&1 | tee -a ${_log_file_name}
            echo "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^" 2>&1 | tee -a ${_log_file_name}
