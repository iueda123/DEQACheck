...
* N列要素
    ...
    * 複数情報が併記されていることがあります。これは論文中に記載されているnormative
      modelが複数ある場合、異なる解析Phaseでの値をメモしているためです。「何番目のモデルか」「モデルの名前（わかる場合）」「解析Phese（Overall
      or Train）」などの場合があります。どのように統一的に表現するか決めかねています。
    * stated は exact と同義。exactに統一
* RC Age列について
    * mean sd median iqr min max の情報を集めたいです。不明場合は数値ではなく NR としたい。例えば 「functioinal models;
      Whole; 22000 (approx); 39.85 (weighted); 7.94 (pooled); 2 (inffered); 100 (inffered)
    ...
    * 数字にコンマが混じっていることがありますがこれは取り除いてください。
      *Whole は Overall と同義。Overallに統一
    * 複数情報が併記されていることがある。これは論文中に記載されているnormative
      modelが複数ある場合、異なる解析Phaseでの値をメモしているためです。「何番目のモデルか」「モデルの名前（わかる場合）」「解析Phese（Overall
      or Train）」などの場合があります。どのように統一的に表現するか決めかねています。
    * stated は exact と同義。exactに統一
* Sex列について
...
- 形式:
  `Model: <model_tag> | Phase: <Overall|Train> | mean: <val>[ weighted] | sd: <val>[ pooled] | median: <val|NR> | iqr: <val|NR> | min: <val|NR>[ inferred] | max: <val|NR>[ inferred]`
...
- N: `NM1 (functioinal models) 22000 approx Overall` / `NM1 (functioinal models) 11889 exact Train` /
  `NM2 (structural models) 58000 approx Overall` / `NM2 (structural models) 29418 exact Train`
- RC Age: `functioinal models; Whole; 22000 (approx); 39.85 (weighted); 7.94 (pooled); 2 (inffered); 100 (inffered)` など
  4 レコード
...
- SummaryView のセルは文字列ベースのため、複数レコードは改行区切りが視認性・実装容易性のバランスが良い（CSS で
  `white-space: pre-wrap` を検討）。
